...
- every Maven `pom.xml`: `compile` (including the default), `runtime`, and `provided` dependencies, including dependencies in profiles; local parent properties, project/parent aliases, and dependency-management scopes are resolved when their POMs are available in the repository;
...
Test/dev dependencies, Maven parents, dependency-management imports, build plugins, and Gradle platform/project/file dependencies are excluded. Dependencies proven to resolve to a participating Maven module or declared npm/Yarn workspace in the same repository are also excluded because they are code under the current review, not previously published library acceptance. Coordinate names alone do not establish locality: Maven versions must match, and npm workspace protocols, local paths, or compatible workspace versions must prove local resolution. Ambiguous local resolution requires manual review unless the coordinate is already accepted by the ledger.

S009 intentionally checks direct declarations only; lockfiles and transitive dependency graphs are not approval evidence, so a missing lockfile does not make the result incomplete. Reports retain each declaration as evidence, while summaries count distinct ecosystem-qualified coordinates.
...
- `MANUAL`: a dependency project exists but the installed ledger is missing, malformed, non-authoritative, or internally inconsistent; same-repository resolution for an otherwise unaccepted coordinate is ambiguous; or no definite violation exists but dependency evidence is incomplete.
...
The checked-in ledger is authoritative and contains the TC-reviewed module and [library baseline](s009-library-baseline.md). Only its exact `libraryCoordinates` are accepted. Every other external `org.folio:*` or `@folio/*` production dependency fails until the Technical Council explicitly adds that package coordinate. Same-repository exclusion only establishes that a dependency is code under the current review; it is not an acceptance decision. Existing family records, `moduleIdentities`, S008 catalog providers, FAR presence, module-registry publication, Platform inclusion, repository ownership, and package publication do not imply package acceptance.
